...
function topological_sort(g) {
	...
	var amendedEdges = [];
   var ret = topological_sort_helper(g, amendedEdges);

    //Cleanup: Remove the deleted property
    for(i in g.nodes)
	delete g.nodes[i].deleted

//	// reverse amended edges.
//	for(var ofst = 0;ofst < amendedEdges.length;ofst++) {
//		var edge = amdendedEdges[ofst];
...
function topological_sort_helper(g, amendedEdges) {
	//Find node with no incoming edges
	var node;
	while(node === undefined) {
		var firstUndeletedEdge = null;
		for(i in g.nodes) {
			...
			for(j in g.nodes[i].edges) {
				...
				&& g.nodes[i].edges[j].source.deleted == false) {
					incoming = true;
					break;
				} else if(firstUndeletedEdge === null) {
					var possibleEdge = g.nodes[i].edges[j];
					for(var ofst = 0;ofst < amendedEdges.length;ofst++) {
						if(amendedEdges[ofst] === possibleEdge) {
							possibleEdge = null;
							break;
						}
					}
					...
					if(possibleEdge != null) {
						firstUndeletedEdge = possibleEdge;
					}
				}
			}
			if(!incoming) {
		...
		// Either recursive or done.
		if(node == undefined) {
			if(firstUndeletedEdge != null) {
				// a recursive loop was encountered.
				// reverse this arbitrarily selected edge and
				// repeat the sort
				var oldSource = firstUndeletedEdge.source;
				firstUndeletedEdge.source = firstUndeletedEdge.target;
				firstUndeletedEdge.target = oldSource;
				amendedEdges.push(firstUndeletedEdge);
			} else 
				return [];
		}
	...
	var tail = topological_sort_helper(g, amendedEdges);
